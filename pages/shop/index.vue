<template>
	<div class="container mx-auto my-3">
		...
		<div class="flex flex-wrap justify-center">
			<div v-for="p in product" class="m-1 hover:shadow-xl w-64 cursor-pointer border b-gray-400 rounded flex flex-col justify-center items-center text-center p-6 bg-white">
			  ...
			  <p class="text-gray-700 mb-2">{{ p.name }}</p>
			  <div class="text-3xl font-bold text-gray-900 mb-6">BDT {{ p.price }}</div>
...
export default {
	...
	async asyncData ({ params }) {
		let product = await axios.get(`/api/shopproduct`)
		return {
